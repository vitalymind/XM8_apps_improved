...
/*
Lets fill our slider with contents
...
_makeButtonCtrl = {
	...
	_ctrl ctrlSetPosition [_pos select 0,_pos select 1,6 * _pW, 5 * _pH];
	_ctrl ctrlSetText _text;
	_ctrl ctrlSetEventHandler ["ButtonClick",format ["call %1",_script]];
	_ctrl ctrlCommit 0;
...
/* 
Lets try to create 13 buttons, if some buttons in XM8_apps_config are commented,
they will be scipped
*/
for "_i" from 1 to 13 do {
	...
	if (!isNil (format ["XM8_apps_app%1", _i])) then {
		...
		if (_i in [1,2,3,4,5]) then {
			_pos set [0, ((4 + ((_i-1) * 6.5))- 3) * _pW];
			_pos set [1, (4 - 2) * _pH];
		};
		if (_i in [6,7,8,9,10]) then {
			_pos set [0, ((4 + (((_i-1) - 5) * 6.5))- 3) * _pW];
			_pos set [1, (9.5 - 2) * _pH];
		};
		if (_i in [11,12,13]) then {
			_pos set [0, ((4 + (((_i-1) - 10) * 6.5))- 3) * _pW];
			_pos set [1, (15 - 2) * _pH];
		};
		[(_newSliderIDC + 1 + _i) ,_pos,(_data select 0),str(_data select 2),_newSlider,(_data select 1)] call _makeButtonCtrl;
	};
};
